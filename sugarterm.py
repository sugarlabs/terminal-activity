...
from sugar4 import env, profile
...
class SugarTerminal(Vte.Terminal):
    ...
    def __init__(self, activity):
        ...
        self.handler_ids = []
        self.read_config()
        # GTK4: Use click gesture controller instead of button-press-event
    ...
    def copy_clipboard(self, widget=None, content=None):
        if self.get_has_selection() and (content is None):
            ...
            # GTK4: Clipboard access is different
    ...
    def add_matches(self):
        ...
        try:
            ...
            for expr in TERMINAL_MATCH_EXPRS:
                ...
                # GTK4: match_set_cursor_type -> match_set_cursor_name
                try:
                    self.match_set_cursor_name(tag, "pointer")
                except AttributeError:
                    # Fallback for older VTE versions
    ...
    def _on_button_pressed(self, gesture, n_press, x, y):
        """GTK4: Handle button press via gesture controller"""

        # Create a fake event-like object for compatibility
        class FakeEvent:
            ...
            def get_state(self):
                return Gdk.ModifierType(self._state)

        event = FakeEvent(x, y, gesture.get_current_button())
        self.button_press(self, event)
    ...
    def button_press(self, terminal, event):
        ...
        """
        self.matched_value = ""
        # GTK4: match_check_event doesn't exist, use match_check with coordinates
        try:
            matched_string = self.match_check_event(event)
        except AttributeError:
            # Fallback to coordinate-based checking
    ...
    def set_term_colors(self, custom_colors):
        ...
        try:
            ...
            try:
                # Fallback for older VTE versions
                self.set_colors(
                    Gdk.color_parse(fg_color), Gdk.color_parse(bg_color), []
                )
            except AttributeError:
                pass
    ...
    def browse_link_under_cursor(self):
        if not self.found_link:
            ...
            return
        path = os.path.join(
            self.activity.get_activity_root(), "instance", "%i" % time.time()
        )
        fd = open(path, "w")
        ...
        journal_entry.metadata["description"] = "Opening {} from the Terminal".format(
            self.found_link
        )
